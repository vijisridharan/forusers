                ...
                <div class="card-body">
                  ...
                  <form method="POST" action="{{ url('/editPost',array($posts->id)) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <textarea id="post_body" rows="6" class="form-control @error('post_body') is-invalid @enderror" name="post_body" required autocomplete="post_body">{{$posts->post_body}}</textarea>

                                @error('post_body')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $error->first('post_body')}}</strong>
                                    </span>
                                @enderror
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            <label for="category_id" class="col-md-6 col-form-label text-md-right">{{ __('Category') }}</label>
